import { delay, getUniqueID, RANDOMNAMES, shuffle, requiredNum } from "./util.js";
...
class PlayerArray extends Array {
    alive() {
...
export default class MafiaGame {
    uid = getUniqueID();
    playersByRole = new DefaultMap(() => new PlayerArray());
    players;
    endGame;
    send;
    disconnected;
    ...
    async dealNames() {
        ...
        const nameIs = (msg, p) => {
            if (msg.length > 20 || msg.length < 3)
                this.sendQuestion("sorry? (name must have 3-20 characters)", nameIs, p);
            else if (usedNames.includes(msg.toLowerCase()))
                this.sendQuestion("sorry? (name unavailable)", nameIs, p);
            else {
                p.name = msg;
    ...
    async loop() {
        ...
        while (!this.ended) {
            ...
            if (night == 1)
                this.bcText("Remember, you can refer to players by their name or number.");
            await delay(delayTime);
            await this.night();
            await delay(delayTime);
    ...
    async day() {
        ...
        this.chatting = true;
        let skipchat = new Promise((resolve, reject) => {
            let votes = 0;
            this.skipVote = (p) => {
                console.log("Skip votes: " + ++votes);
                this.sendText(`${votes} voted to skip.`, p);
                if (votes >= this.players.alive().length)
                    resolve();
            };
        });
        ...
        await delay(delayTime);
        const votes = new DefaultMap(() => 0);
        const lynchVote = (msg, p) => {
            let player = this.findPlayer(msg);
            if (!player)
                this.sendQuestion("sorry?", lynchVote, p);
            else if (player == p)
                this.sendQuestion("sorry? (vote for someone else)", lynchVote, p);
            else
                votes.set(player, votes.get(player) + 1);
        };
        ...
        const highestVotes = votesArr[0];
        const noVotes = this.players.alive().length - votesArr.reduce((p, c) => p + c[1], 0);
    ...
    async night() {
        const mafiaKills = (victimName, mafioso) => {
            ...
            if (!victim)
                this.sendQuestion(`sorry?`, mafiaKills, mafioso);
        ...
        if (remainingMafia) {
            this.gfIndex++;
            if (this.gfIndex >= remainingMafia.length)
                this.gfIndex = 0;
