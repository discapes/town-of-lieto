...
export default class Game {
	...
	async start() {
		...
		const nameIs = (msg, p) => {
			if (msg.length >= 20) this.sendQuestion("sorry? (must have <=20 characters)", nameIs, ...this.players);
			p.name = msg;
		};
	...
	async night() {
		const mafiaKills = (victimName, mafioso) => {
			...
			if (!victim) this.sendQuestion(`sorry?`, mafiaKills, ...this.playersByRole.get("mafioso"));
